...
export const pSBC = (p: number, c0: string, c1?: string, l?: boolean): string => {
    let r,g,b,P,f,t,h,i=parseInt,m=Math.round,a=typeof(c1)=="string";
    if(typeof(p)!="number"||p<-1||p>1||typeof(c0)!="string"||(c0[0]!='r'&&c0[0]!='#')||(c1&&!a))return c0;
    if(!aux.pSBCr)aux.pSBCr=(d: any)=>{
        let n=d.length,x={};
        if(n>9){
    ...
    h=c0.length>9,h=a?c1.length>9?true:c1=="c"?!h:false:h,f=aux.pSBCr(c0),P=p<0,t=c1&&c1!="c"?aux.pSBCr(c1):P?{r:0,g:0,b:0,a:-1}:{r:255,g:255,b:255,a:-1},p=P?p*-1:p,P=1-p;
    if(!f||!t)return c0;
